...
#define UAVS3D_CHECK_VAILD_RANGE(v, l, r) ((v)>=(l)&&(v)<=(r))
    ...
    if (!buf_size) {
        ...
        while (!finish) {
            ...
            if (frm_dec->nal_type == NAL_SEQ_HEADER) {
                ...
                if (!UAVS3D_CHECK_VAILD_RANGE(seqh->frame_rate_code, 0, 15) {
                ...
                if (seqh->colour_description) {
                    if (!UAVS3D_CHECK_VAILD_RANGE(seqh->colour_primaries, 0, 9) ||
                        !UAVS3D_CHECK_VAILD_RANGE(seqh->transfer_characteristics, 0, 14) ||
                        !UAVS3D_CHECK_VAILD_RANGE(seqh->matrix_coefficients, 0, 11)) {
